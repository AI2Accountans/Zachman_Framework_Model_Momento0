...
Nodos del Gemelo Digital Semántico activos.`
    ...
    // ---- Momento 0 Step-by-Step Flow Walkthrough ----
    const startWkBtn = document.getElementById('start-walkthrough');
    const wkPanel = document.getElementById('walkthrough-panel');
    const wtTitle = document.getElementById('wt-step-title');
    const wtDesc = document.getElementById('wt-step-desc');
    ...
    const steps = [
        ...
        {
            cellId: 'B4',
            title: '2. Ingesta: Clientes y Scripts (B4)',
            desc: 'Los programas de ingesta (en la capa Física y columna de Proceso/How) abren los XMLs y extraen los campos de interés operativo para organizarlos en tablas intermedias como data.csv.'
        },
        {
            cellId: 'B3',
            title: '3. Mapeo Semántico: Altova MapForce (B3)',
            desc: 'A nivel Lógico (RDF Graph), los datos planos se mapean en MapForce y se traducen a la estructura de grafo JSON-LD, usando la taxonomía estándar de XBRL GL como diccionario.'
        },
        {
            cellId: 'C2',
            title: '4. Semántica: Taxonomías XBRL GL (C2)',
            desc: 'En la capa Conceptual (Ontology OWL), se importan las taxonomías oficiales globales de XBRL GL y ESG. Esto garantiza que conceptos como "accountMainID" tengan un significado idéntico e integrable en todo el mundo.'
        },
        {
            cellId: 'C3',
            title: '5. Modelado Lógico: Modelos JSON-LD (C3)',
            desc: 'Los datos mapeados se estructuran en el modelo lógico JSON-LD final (Capa RDF, columna What) representando las cuentas y transacciones con URIs de herencia directa.'
        },
        {
            cellId: 'A3',
            title: '6. Validación: Integrity SHACL (A3)',
            desc: 'El validador SHACL (Shapes Constraint Language) en la columna de Motivación (Why) ejecuta las reglas del negocio. La base de datos rechaza transacciones desbalanceadas o que carezcan de enlaces a agentes/contratos.'
        },
        {
            cellId: 'C6',
            title: '7. Grafo Contable: Instancias en TerminusDB (C6)',
            desc: 'A nivel de Ejecución (Functioning / Linked Data Assets), los registros validados se inyectan en TerminusDB/DFRNT, consolidando el libro contable de grafos como la única fuente de verdad (SSOT).'
        },
        {
            cellId: 'C1',
            title: '8. Confianza y Anclaje: Auditoría por Diseño (C1)',
            desc: 'A nivel Contextual (Trust & UI), el "Momento Cero" de constitución se firma y ancla criptográficamente en la blockchain (Algorand). Además, la ontología PROV-O de la W3C garantiza la trazabilidad completa del balance final de vuelta al XML original.'
        }
    ];
    ...
    function showStep(idx) {
        ...
        // Highlight active step cell
        const step = steps[idx];
        const targetCell = document.querySelector(`.holon-cell[data-cell="${step.cellId}"]`);
        if (targetCell) {
            targetCell.classList.remove('faded');
        ...
        // Update panel
        wtTitle.innerText = step.title;
        wtDesc.innerText = step.desc;

        // Button disabled states
        wtPrev.disabled = (idx === 0);
        wtNext.innerText = (idx === steps.length - 1) ? 'Finish' : 'Next';
    }

    function exitWalkthrough() {
        walkthroughActive = false;
        wkPanel.style.display = 'none';
        startWkBtn.innerText = 'Start Flow Walkthrough';
        startWkBtn.classList.remove('btn-primary');
        ...
        applyFilters(); // Re-apply existing sidebar filters
    }

    startWkBtn.addEventListener('click', () => {
        if (walkthroughActive) {
            exitWalkthrough();
        } else {
            walkthroughActive = true;
            currentStepIdx = 0;
            wkPanel.style.display = 'block';
            startWkBtn.innerText = 'Exit Walkthrough';
            startWkBtn.classList.add('btn-primary');
